...
bool MaglevCompiler::Compile(LocalIsolate* local_isolate,
  ...
  bool is_tracing_enabled = false;
  {
    ...
    // When collecting source positions for profiling (e.g. --perf-prof
    // --detailed-line-info), create a graph labeller so per-node provenance
    // (including inlined positions) is recorded during graph building and a
    // real source position table can be attached to the Code object. Upstream
    // V8 folds this into the single graph-labeller condition; node's V8 13.6
    // creates the labeller in a differently-shaped block, so the condition is
